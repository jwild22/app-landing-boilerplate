...
- Screenshots are the live App Store set (source of truth: `ExamMaster/screenshots/final/en/`, native 1290×2796). If they change, re-export at 1080px wide, JPEG q85, replace `assets/screenshot-*.jpg`, **and bump the `?v=N` query on every screenshot reference** (`sed -i '' -E 's#(/assets/screenshot-[a-z0-9-]+\.jpg)\?v=[0-9]+#\1?v=N#g' *.html fr/*.html guides/*.html`) — the `/assets/` files carry a 1-year immutable cache, so same-name overwrites won't reach cached clients without a new version.
